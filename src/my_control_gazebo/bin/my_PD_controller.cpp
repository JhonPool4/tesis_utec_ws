...
// Effort control class: proportional-derivative control method 
namespace effort_controllers_ns{
	...
	class ArticularPDController : public controller_interface::Controller<hardware_interface::EffortJointInterface>
	{
		private:
			...
			realtime_tools::RealtimeBuffer<std::vector<double> > _position_command;
			// Real time buffer: angular velocity
			realtime_tools::RealtimeBuffer<std::vector<double> > _velocity_command;
			// Subscriber
			ros::Subscriber _sub_position_command;
			ros::Subscriber _sub_velocity_command;
			// Publisher
			ros::Publisher _pub_interface_command;
			ros::Publisher _pub_current_position;
			ros::Publisher _pub_current_velocity;
			// Control gains
			Vector_d_6x1 _kp;
			Vector_d_6x1 _kd;
			// RBDL
			...
			int _p_rate = 100;
			// start flag
			std_msgs::Int64 _f_start;
			// current joint states
			my_control_gazebo::AngularPosition _joint_position;
			my_control_gazebo::AngularVelocity _joint_velocity;
		public:
			bool init(hardware_interface::EffortJointInterface* hw, ros::NodeHandle &n)
			{
				...
				_position_command.writeFromNonRT(std::vector<double>(6, 0.0));
				_velocity_command.writeFromNonRT(std::vector<double>(6, 0.0));

				// Load urdf model
				if (!Addons::URDFReadFromFile ("/home/jhon/catkin_ws/tesis_utec_ws/src/ur5_description/urdf/ur5_joint_limited_robot.urdf", _model, false))
                {
					/* Do nothing*/
				}

				// Subscribers
				_sub_position_command = n.subscribe<my_control_gazebo::AngularPosition>("position_command", 1, &ArticularPDController::positionCommandCB, this);
				_sub_velocity_command = n.subscribe<my_control_gazebo::AngularVelocity>("velocity_command", 1, &ArticularPDController::velocityCommandCB, this);

				// Publishers
				_pub_interface_command = n.advertise<std_msgs::Int64>("f_start", 1);
				_pub_current_position = n.advertise<my_control_gazebo::AngularPosition>("current_position", 1);
				_pub_current_velocity = n.advertise<my_control_gazebo::AngularVelocity>("current_velocity", 1);
				return true;					
			}
			...
			void positionCommandCB(const my_control_gazebo::AngularPosition::ConstPtr& msg_p){
				...
				_position_command.initRT(desired_position); // ee_pose_command_ require std::vector<double>	
			}
			...
			void velocityCommandCB(const my_control_gazebo::AngularVelocity::ConstPtr& msg_v){
				...
				_velocity_command.initRT(desired_velocity); // ee_pose_command_ require std::vector<double>										
			}
			...
			void update(const ros::Time& time, const ros::Duration& period)
			{
				...
				std::vector<double> & des_angular_position = *_position_command.readFromRT();
				std::vector<double> & des_angular_velocity = *_velocity_command.readFromRT();
				...
				{										
					...
					p_term[i]  = _kp[i]*p_error[i];
					// Derivative term
					d_error[i] = des_angular_velocity[i] - dq[i];
					d_term[i]  = _kd[i]*d_error[i];	
				}
				
				// Robot dynamics
				CompositeRigidBodyAlgorithm(*_model, q, M); // inertia matrix
				NonlinearEffects(*_model, q, dq, b);		// nonlinear effects vector
				...
				_joint_position.q1= q[0];
				_joint_position.q2= q[1];
				_joint_position.q3= q[2];
				_joint_position.q4= q[3];
				_joint_position.q5= q[4];
				_joint_position.q6= q[5];
				_pub_current_position.publish(_joint_position);
				// send current velocity
				_joint_velocity.dq1 = dq[0];
				_joint_velocity.dq2 = dq[1];				
				_joint_velocity.dq3 = dq[2];
				_joint_velocity.dq4 = dq[3];
				_joint_velocity.dq5 = dq[4];
				_joint_velocity.dq6 = dq[5];
				_pub_current_velocity.publish(_joint_velocity);
				...
				_pub_interface_command.publish(_f_start);
