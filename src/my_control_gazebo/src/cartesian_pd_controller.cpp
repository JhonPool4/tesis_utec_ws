...
namespace effort_controllers_ns{
	...
	class CartesianPDController : public controller_interface::Controller<hardware_interface::EffortJointInterface>
	{
        // type: effort signal
        // space: task-space 
        ...
        public:
			bool init(hardware_interface::EffortJointInterface* hw, ros::NodeHandle &n)
			{
				...
				// Subscribers
				_sub_pose_command = n.subscribe<my_control_gazebo::Pose>("pose_command", 1, &CartesianPDController::poseCommandCB, this);
				// Publishers
            ...
            void update(const ros::Time& time, const ros::Duration& period)
			{
				...
				//u = eval_control_limits(J.transpose()*F + b);
				u = J.transpose()*F;
				...
				if (true)
				{
					...
					if (_counter%_p_rate == 0){
						...
						std::cout<<"\nnorm: "<<x_e.block<3,1>(0,0).norm()<<std::endl;
	...
	PLUGINLIB_EXPORT_CLASS(effort_controllers_ns::CartesianPDController, controller_interface::ControllerBase);
}    